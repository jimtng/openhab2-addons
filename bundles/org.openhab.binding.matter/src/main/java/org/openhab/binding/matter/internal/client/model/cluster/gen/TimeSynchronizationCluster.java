...
public class TimeSynchronizationCluster extends BaseCluster {
    ...
    /**
     * A Node ID, endpoint, and associated fabric index of a Node that may be used as trusted time source. See Time
     * source prioritization. This attribute reflects the last value set by an administrator using the
    ...
    /**
     * This command shall set the TrustedTimeSource attribute. Upon receipt of this command, * If the TrustedTimeSource
     * field in the command is null, the node shall set the TrustedTimeSource attribute to null and shall generate a
     * MissingTrustedTimeSource event. * Otherwise, the node shall set the TrustedTimeSource attribute to a struct which
     * has NodeID and Endpoint fields matching those in the TrustedTimeSource field and has its FabricIndex field set to
