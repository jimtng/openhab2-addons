...
public class AdministratorCommissioningCluster extends BaseCluster {
    ...
    /**
     * Indicates whether a new Commissioning window has been opened by an Administrator, using either the OCW command or
     * the OBCW command.
     * This attribute shall revert to WindowNotOpen upon expiry of a commissioning window.
     * Note that an initial commissioning window is not opened using either the OCW command or the OBCW command, and
     ...
     * The current Administrator MUST specify a timeout value for the duration of OCW.
     * When OCW expires or commissioning completes, the Node shall remove the Passcode by deleting the PAKE passcode
     * verifier as well as stop publishing the DNS-SD record corresponding to this command as described in Section
     * 4.3.1, “Commissionable Node Discovery”. The commissioning into a new Fabric completes when the Node successfully
    ...
    /**
     * This command is used by a current Administrator to instruct a Node to revoke any active Open Commissioning Window
     * or Open Basic Commissioning Window command. This is an idempotent command and the Node shall (for ECM) delete the
     * temporary PAKEPasscodeVerifier and associated data, and stop publishing the DNS-SD record associated with the
     * Open Commissioning Window or Open Basic Commissioning Window command, see Section 4.3.1, “Commissionable Node
     * Discovery”.
     ...
     * immediately expire the fail-safe and perform the cleanup steps outlined in Section 11.10.6.2.2, “Behavior on
