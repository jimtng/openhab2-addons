...
public class LaundryWasherControlsCluster extends BaseCluster {
    ...
    /**
     * This attribute indicates the list of spin speeds available to the appliance in the currently selected mode. The
     * spin speed values are determined by the manufacturer. At least one spin speed value shall be provided in the
     * SpinSpeeds list. The list of spin speeds may change depending on the currently selected Laundry Washer mode. For
    ...
    /**
     * This attribute indicates the currently selected spin speed. It is the index into the SpinSpeeds list of the
     * selected spin speed, as such, this attribute can be an integer between 0 and the number of entries in SpinSpeeds
     * - 1. If a value is received that is outside of the defined constraints, a CONSTRAINT_ERROR shall be sent as the
     * response. If a value is attempted to be written that doesn’t match a valid index (e.g. an index of 5 when the
     * list has 4 values), a CONSTRAINT_ERROR shall be sent as the response. If null is written to this attribute, there
     * will be no spin speed for the selected cycle. If the value is null, there will be no spin speed on the current
     * mode.
     */
    public Integer spinSpeedCurrent; // 1 uint8 RW VO
    /**
     * This attribute represents how many times a rinse cycle shall be performed on a device for the current mode of
     * operation. A value of None shall indicate that no rinse cycle will be performed. This value may be set by the
     * client to adjust the number of rinses that are performed for the current mode of operation. If the device is not
     * in a compatible state to accept the provided value, an INVALID_IN_STATE error shall be sent as the response.
     */
     ...
     * NumberOfRinsesEnum value that is possible in the selected mode on the device. The value of this attribute may
     * change at runtime based on the currently selected mode. Each entry shall be distinct.
     */
