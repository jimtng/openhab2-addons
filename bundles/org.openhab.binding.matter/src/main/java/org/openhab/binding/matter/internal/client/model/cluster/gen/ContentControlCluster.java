...
public class ContentControlCluster extends BaseCluster {
     ...
     * this attribute equals FALSE, then playback of unrated content
     ...
     * the PINCode passed in the command matches the current PINCode value. If these match, then the RemainingScreenTime
     ...
     * shall check if there is an entry in the BlockContentTimeWindow attribute which matches with the TimePeriod and
     * DayOfWeek fields passed in this command. * If Yes, then a response with TimeWindowAlreadyExist error status shall
     * be returned. * If No, then the media device shall assign one unique index for this time window and add it into
